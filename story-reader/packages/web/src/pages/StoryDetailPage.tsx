  ...
  return (
    <Layout showBack title={story.title} hideNav={false}>
      ...
      <div className="relative">
        <div className="relative overflow-hidden h-40 md:h-52 lg:h-64">
        ...
        <div className="lg:grid lg:grid-cols-[minmax(0,1fr)_420px] xl:grid-cols-[minmax(0,1fr)_480px] lg:gap-6 lg:items-start px-4 lg:px-8 xl:px-10 -mt-4 relative max-w-screen-xl">

          {/* LEFT COLUMN: info + actions + shelf */}
          <div>
            {/* Cover + Info */}
            <div className="flex gap-4">
              <img
                src={story.cover}
                alt={story.title}
                className="w-24 h-36 lg:w-36 lg:h-52 object-cover rounded-xl shadow-lg flex-shrink-0 -mt-12 lg:-mt-20 border-2 border-white"
              />
              <div className="flex-1 pt-1 min-w-0">
                <h1 className="text-lg lg:text-2xl font-bold text-gray-900 leading-tight line-clamp-2">{story.title}</h1>
            ...
            {/* Description */}
            <div className="mt-4">
              <div className={`text-sm text-gray-700 leading-relaxed ${!descExpanded ? 'line-clamp-3 lg:line-clamp-none' : ''}`}>
                {story.description}
              </div>
              <button
            ...
            {/* Chapter list — mobile only (shown in right col on desktop) */}
            <div className="mt-6 mb-4 lg:hidden">
              <ChapterListSection
                ...
                setChapterSortDesc={setChapterSortDesc}
                chapterPage={chapterPage}
                setChapterPage={setChapterPage}
                chapterPageCount={chapterPageCount}
                safeChapterPage={safeChapterPage}
          ...
          {/* RIGHT COLUMN — desktop only: chapters + comments */}
          <div className="hidden lg:block mt-4 lg:mt-0 pb-8">
...
interface ChapterListSectionProps {
  ...
  setChapterSortDesc: (fn: (prev: boolean) => boolean) => void;
  chapterPage: number;
  setChapterPage: (fn: (prev: number) => number) => void;
  chapterPageCount: number;
  safeChapterPage: number;
          ...
          return (
            <Link
              ...
              className={`flex items-center justify-between px-4 py-3 rounded-xl transition-colors
                ${isCurrentChapter ? 'bg-primary-50 border border-primary-100' : 'bg-white border border-gray-100 hover:bg-gray-50'}`}
            >
